...
char useable_flag;
char auto_hot;
unsigned int white_const,red_const[10],green_const[10],blue_const[10];
unsigned char ch;                           /*通道号*/
unsigned char DEV_temp,FIX_temp,DRY_temp;
unsigned char size;                         /*镜头.片框位置:0-1";1-5";2-7"*/
unsigned int sum_page;                  /*总张数累计*/
unsigned int white_agfa[10],red_agfa[10],green_agfa[10],blue_agfa[10];
unsigned int white_agfa_c[10],red_agfa_c[10],green_agfa_c[10],blue_agfa_c[10];
char stability;      /*  AD稳定性判断因子的倒数  */
unsigned int mid_r[10],mid_g[10],mid_b[10],mid_w[10];             /*红,绿,蓝中间点密度*/
char flag2;
#pragma memory=default
...
set_ad(char gain)
{
    ...
    AD=2;                       /*增益设置到32*/
    write_ad(AD|SETUP|W);
    write_ad(0x64);             /*写AD7715设置寄存器,自校准,2.4576MHz晶振,50Hz更新速率,单极性工作,无输入缓冲,非同步滤波器*/
/*    do{
        delay(200);
        write_ad(AD|READ);
        nn=read_ad_res();
    ...
    while(key()!=go_key){
        if(key()==r_key){
            ...
            while(key()==INVAILD){
                ...
                gotoxy(1,88);
                printf("%d  ",get_ad());
                aa= (unsigned int) (log10((double)red_base/(double)get_ad())*1000);
        ...
        if(key()==g_key){
            ...
            while(key()==INVAILD){
                ...
                gotoxy(1,88);
                printf("%d  ",get_ad());
                aa= (unsigned int) ((double)log10((double)green_base/(double)get_ad())*1000);
                lcd_asc(3,64,aa/10000+48);
                lcd_asc(3,72,(aa/1000)%10+48);
                lcd_asc(3,80,(aa/100)%10+48);
                lcd_asc(3,88,(aa/10)%10+48);
...
/*            set(filter,ON);*/
            ...
            while(key()==INVAILD){
                ...
                gotoxy(1,88);
                printf("%d  ",get_ad());
                aa= (unsigned int) (log10((double)blue_base/(double)get_ad())*1000);
                lcd_asc(3,64,aa/10000+48);
                lcd_asc(3,72,(aa/1000)%10+48);
                lcd_asc(3,80,(aa/100)%10+48);
                lcd_asc(3,88,(aa/10)%10+48);
        ...
        if(key()==uiji_key){
            ...
            while(key()==INVAILD){
                ...
                gotoxy(1,88);
                printf("%d  ",get_ad());
                aa= (unsigned int) (log10((double)white_base/(double)get_ad())*1000);
                lcd_asc(3,64,aa/10000+48);
                lcd_asc(3,72,(aa/1000)%10+48);
                lcd_asc(3,80,(aa/100)%10+48);
                lcd_asc(3,88,(aa/10)%10+48);
...
detect()
{
    double sen_w,sen_r,sen_g,sen_b,agfa;
    set(r_m,ON);
...
/*            r_time=((double)red_const2/(pow(red,2)))*10000;
            ...
            if(r_time>MAXR)r_time=MAXR;
            if(g_time>MAXG)g_time=MAXG;
            if(b_time>MAXB)b_time=MAXB;
            gotoxy(0,0);
            lcd_cls(0,0,4,128);
            printf("g:%d,a:%d S:%d",green,green_agfa[ch],(int)(sen_g*10000));
            printf("b:%d m:%d,c:%d",green_base,mid_g[ch],green_const[ch]);
            wait_key();
            lcd_cls(0,0,4,128);*/
            senr=sen_r*1000;
            seng=sen_g*1000;
            senb=sen_b*1000;
            if(senr>=mid_r[ch])agfa=(double)red_agfa[ch]/100-1;else agfa=(double)red_agfa_c[ch]/100-1;
            r_time=pow(10,agfa*((double)senr-(double)mid_r[ch])/1000+(double)red_const[ch]/1000)/(double)red;
            fr=(agfa*((double)senr-(double)mid_r[ch])/1000)*1000+red_const[ch];
/*            gotoxy(0,0);
            lcd_cls(0,0,4,128);
            printf("RS:%d,RM%d,a%d,ac%d,rc%d,rg%d,fr%d,rt%d,ag%d",senr,mid_r[ch],red_agfa[ch],red_agfa_c[ch],red_const[ch],red,fr,r_time,(int)(agfa*100));
            wait_key();
            gotoxy(0,0);
            lcd_cls(0,0,4,128);*/
            if(seng>=mid_g[ch])agfa=(double)green_agfa[ch]/100-1;else agfa=(double)green_agfa_c[ch]/100-1;
            g_time=pow(10,agfa*((double)seng-(double)mid_g[ch])/1000+(double)green_const[ch]/1000)/(double)green;
            fg=(agfa*((double)seng-(double)mid_g[ch])/1000)*1000+green_const[ch];
            if(senb>=mid_b[ch])agfa=(double)blue_agfa[ch]/100-1;else agfa=(double)blue_agfa_c[ch]/100-1;
            b_time=pow(10,agfa*((double)senb-(double)mid_b[ch])/1000+(double)blue_const[ch]/1000)/(double)blue;
            fb=(agfa*((double)senb-(double)mid_b[ch])/1000)*1000+blue_const[ch];
            if(r_time>MAXR)r_time=MAXR;
            if(g_time>MAXG)g_time=MAXG;
            if(b_time>MAXB)b_time=MAXB;
        }
...
{
    int rr,gg,bb;
    double sen_r,sen_g,sen_b;
    for(;page>=1;page--){
        set(r_m,ON);
        delay(DELAY);
        red=get_ad();
        sen_r=(double)log10((double)red_base/(double)red);
        r_time=(double)pow(10,(double)((double)((sen_r>(double)mid_r[ch]/1000)?red_agfa[ch]:red_agfa_c[ch])/100-1)*(sen_r-(double)mid_r[ch]/1000)+(double)red_const[ch]/1000)/(double)red;
    if(r_time>MAXR)r_time=MAXR;
        rr=(double)r_time*(1+(double)(r_scale+d_scale-20)/10);
        set(door,ON);
        time=0;
        ...
        green=get_ad();
        sen_g=(double)log10((double)green_base/(double)green);
        g_time=(double)pow(10,(double)((double)((sen_g>(double)mid_g[ch]/1000)?green_agfa[ch]:green_agfa_c[ch])/100-1)*(sen_g-(double)mid_g[ch]/1000)+(double)green_const[ch]/1000)/(double)green;
            if(g_time>MAXG)g_time=MAXG;
        gg=(double)g_time*(1+(double)(g_scale+d_scale-20)/10);
        set(door,ON);
        time=0;
        ...
        blue=get_ad();
        sen_b=(double)log10((double)blue_base/(double)blue);
        b_time=(double)pow(10,(double)((double)((sen_b>(double)mid_b[ch]/1000)?blue_agfa[ch]:blue_agfa_c[ch])/100-1)*(sen_b-(double)mid_b[ch]/1000)+(double)blue_const[ch]/1000)/(double)blue;
            if(b_time>MAXB)b_time=MAXB;
        bb=(double)b_time*(1+(double)(b_scale+d_scale-20)/10);
        set(door,ON);
        set(filter,ON);
...
/*    temp=(int)white_const;
    ...
    while(k!=uiji_key){
        ...
        if(k==page_key){
            ...
            printf("R   G    B");
            gotoxy(1,0);
            printf("%d %d %d",senr,seng,senb);
            gotoxy(2,0);
            printf("%s",msg72);
            if(wait_key()==ch_key){
                write_int(&mid_r[ch],senr);
                write_int(&mid_g[ch],seng);
                write_int(&mid_b[ch],senb);
            }
        }
    }
    temp=(int)red_const[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg54);
    gotoxy(2,0);
    ...
    write_int(&red_const[ch],temp);
    temp=(int)green_const[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg55);
    gotoxy(2,0);
    ...
    write_int(&green_const[ch],temp);
    temp=(int)blue_const[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg56);
    gotoxy(2,0);
...
/*    temp=(int)mid_w;
    ...
    write_int(&mid_r[ch],temp);
    temp=(int)red_agfa_c[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg58);
    gotoxy(2,0);
    ...
    write_int(&red_agfa_c[ch],temp);
    temp=(int)red_agfa[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg73);
    gotoxy(2,0);
    ...
    write_int(&mid_g[ch],temp);
    temp=(int)green_agfa_c[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg59);
    gotoxy(2,0);
    ...
    write_int(&green_agfa_c[ch],temp);
    temp=(int)green_agfa[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg74);
    gotoxy(2,0);
    ...
    write_int(&mid_b[ch],temp);
    temp=(int)blue_agfa_c[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg60);
    gotoxy(2,0);
    ...
    write_int(&blue_agfa_c[ch],temp);
    temp=(int)blue_agfa[ch];
    lcd_cls(0,0,4,128);
    gotoxy(0,0);
    printf("%s",msg75);
    gotoxy(2,0);
...
main()
{
    ...
    if(flag1!=0xaa||flag2!=0x55){
        lcd_cls(0,0,4,128);
        gotoxy(0,0);
        printf("setup error!load default!");
        write(&size,FIVE);
        write(&ch,0);
        write(&workmode,1);
        ...
        write(&auto_hot,0);
        write(&stability,20);
        write_int(&sum_page,0);
        write(&flag1,0xaa);
        write(&flag2,0x55);
        for(nn=0;nn<10;nn++){
            write_int(&red_agfa[nn],100);
            write_int(&green_agfa[nn],100);
            write_int(&blue_agfa[nn],100);
            write_int(&red_agfa_c[nn],100);
            write_int(&green_agfa_c[nn],100);
            write_int(&blue_agfa_c[nn],100);
            write_int(&red_const[nn],5000);
            write_int(&green_const[nn],5000);
            write_int(&blue_const[nn],5000);
        }
    }
    if(lip()){
        ISXI=auto_hot;
